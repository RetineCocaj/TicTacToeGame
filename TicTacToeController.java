...
public class TicTacToeController{
   ...
   public void play(){
      while(model.getPlayersTurn() < 9 && !model.getGameIsWon()){
         ...
         if(turn % 2 == 0){JOptionPane.showMessageDialog(null, "It's X's turn.");}
         else{JOptionPane.showMessageDialog(null, "It's O's turn.");}
         try{
            String row = JOptionPane.showInputDialog("Enter number for ROW{0,1,2}:");
            int r = new Integer(row).intValue();
            String col = JOptionPane.showInputDialog("Enter number for COLUMN{0,1,2}:");
